...
To test the hypothesis that prey behaviour shaped the acquisition of predator expertise, we fitted five different models (Table 1) that predicted the expected probability of capturing the four prey $\mu_i$. All models followed the same basic structure defined as:
...
where $\alpha_k$ is the population-level intercept for parameter $k \in \{a, b, c\}$, $\mathbf{W}_{k,i}$ is a vector of covariates affecting asymptotic parameter $k$ with corresponding coefficients $\boldsymbol{\gamma}_k$, $u_{k,j}$ is a predator-specific random intercept. The covariates included in $\mathbf{W}$ for each model are described in Table 1.
...
This covariance structure allowed us to estimate correlations among individual differences in initial success, rate of gain, and maximum hunting success. For example, we could test if predators that were initially successful also had higher rates of gain in expertise and higher maximum hunting success.

```{r model-summary-table, ft.align = "left"}

# Create model summary table
model_summary <- data.frame(
  model = c("I", "II", "III", "IV", "V", "VI"),
  covariates_y = c(
    "game duration",
    "game duration, prey rank",
    "game duration, prey rank",
    "game duration, prey rank, prey speed",
    "game duration, prey rank, prey speed",
    "game duration, prey rank, prey speed, prey space coverage"
  ),
  covariates_asym = c(
    "—",
    "—",
  ...
  hypothesis = c(
    "Baseline expertise acquisition model with no prey covariates",
    "Prey skill (rank) affects hunting success; includes population-level expertise trend",
    "Prey skill (rank) affects hunting success; individual-level expertise only",
    "Prey skill and movement speed affect hunting success; individual-level expertise only",
    "Prey skill and movement speed affect hunting success; includes population-level expertise trend",
    "All prey characteristics affect hunting success; includes population-level expertise trend"
  ),
...
model_table <- model_summary %>%
  ...
  set_caption(
    caption = "Table 1. Summary of the six generalized asymptotic mixed models (GAMMs) fitted to test how prey behaviour shapes predator expertise acquisition. All models include predator identity as a random effect on the asymptotic parameters (a, b, c). Models with population-level expertise trends include a global smoother for cumulative experience.",
    style = "Table Caption"
  ) %>%
...
We compared the predictive accuracy of all six models using approximate leave-one-out cross-validation with Pareto-smoothed importance sampling [@vehtari.etal2017;@piironen.vehtari2017;@vehtari.etal2022].
...
fit4 <- readRDS(here::here("outputs", "outputs_models", "asym-IV.rds"))
fit5 <- readRDS(here::here("outputs", "outputs_models", "asym-V.rds"))

sum1 <- summary(fit1, robust = TRUE)$splines
sum2 <- summary(fit2, robust = TRUE)$splines
sum3 <- summary(fit3, robust = TRUE)$splines
sum4 <- summary(fit4, robust = TRUE)$splines
...
tab1 <- loo_tab %>%
  ...
  set_caption(
    caption = "Table 2. Leave-one-out (loo) cross-validation results comparing the predictive performance of the six GAMMs estimating the relationship between hunting success and predator experience. All models reported include the game duration as a control. Only differences in smoothing functions and prey covariates are reported.",
    style = "Table Caption"
  ) %>%
  font(fontname = "Times New Roman", part = "all") %>%
  fontsize(size = 10, part = "all") %>%
