...
model_formula <- brmsformula(
  ...
  a ~ 1 + Zprey_avg_rank + (1 | predator_id),   # predator-specific asymptote (on logit scale)
  b ~ 1 + (1 | predator_id),                    # predator-specific start (on logit scale)
  c ~ 1 + Zprey_avg_rank + (1 | predator_id),   # predator-specific rate
  betaduration ~ 1,
  betarank ~ 1,
