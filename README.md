...
The subfolders in the [code folder](./code) are those used in the full pipeline. 
...
3. [Produce figures](./code/code_figures)
...
This workflow is exclusively for `.R` files used to compute the Bayesian `asym` models with the `brms` package (see the [code_models](./code/code_models) folder). 
...
| ---------------------- | ---------------- | -------------------- | ------------------ | ------------------------ | -------------------- |
| Digital Research Alliance of Canada / Fir | AlmaLinux 9.6 (Sage Margay) | Linux 5.14.0-570.22.1.el9_6.x86_64, compiled June 19, 2025 | R                    | [code_models](./code/code_models/) folder | [outputs_models](./outputs) folder** |
---

- I ran all the `asym` models on [Fir](https://docs.alliancecan.ca/wiki/Fir), a computer cluster managed by the [Digital Research Alliance of Canada](https://www.alliancecan.ca/en). 

- To run the models on a cluster other than Fir, simply use the job (`.sh`) files and adapt them to your needs. They are in the [code_models](./code/code_models) folders and have the same name as the `.R` model files.
